...
-- WEBHOOK
-----------------------------------------------------------------------------------------------------------------------------------------
local webhookgarmas = "https://canary.discord.com/api/webhooks/879709228107960362/0SB4hm8Ucgh3rdhgkpiBD1yUMpNQi0D_1DiXBaH9DnXuSq9VvaGvexSDIVlTijMA-LFS"
...
local webhookdropar = "https://discord.com/api/webhooks/838775713951514655/SB00xfFpKRirURKTXYTdm1C80XF4znK24G-EhYvZuxljlopApUF4FznEK9qcVpaom"
...
local webhooksaquear = "https://discord.com/api/webhooks/852278727131136000/yesaAfaWN0DsipX27g7VmiGGm2CUyH18Dm2r4wZotoBIj4yLTCQQPHyDwW46RXg-3XWm"
...
function SendWebhookMessage(webhook,message)
	if webhook ~= nil and webhook ~= "https://discord.com/api/webhooks/840727220405665832/_qlhKxHZkME1m1NZ3wMIuyD1UIhB7tI1R_ZrxfNeEbTcm8oCXxuOQcZITn-Shcof" then
...
RegisterCommand('tratamento',function(source,args,rawCommand)
    local user_id = vRP.getUserId(source)
    if vRP.terPemissao(user_id,"medico.permissao") or vRP.terPemissao(user_id,"dono.permissao") then
    ...
    if vRP.hasPermission(user_id,"medico.permissao") then
        if vRP.request(nplayer,"Deseja receber tratamento no valor de <b>$1500</b>?",30) then
            if vRP.tryFullPayment(nuser_id,1500,(args[1])) then
                ...
                TriggerClientEvent('Notify',source,"sucesso","Recebeu <b>R$1500</b> de <b>"..identityu.name.. " "..identityu.firstname.."</b>.")
                if nplayer then
                    if not vRPclient.isComa(nplayer) then
                        TriggerClientEvent("tratamento",nplayer)
                        TriggerClientEvent("Notify",source,"sucesso","Tratamento no paciente iniciado com sucesso.",10000)
    ...
    if vRP.hasPermission(user_id,"medico.permissao") then
        if vRP.request(nplayer,"Deseja receber tratamento no valor de <b>$0</b>?",30) then
            if vRP.tryFullPayment(nuser_id,0,(args[1])) then
                ...
                TriggerClientEvent('Notify',source,"sucesso","Recebeu <b>R$0</b> de <b>"..identityu.name.. " "..identityu.firstname.."</b>.")
                if nplayer then
                    if not vRPclient.isComa(nplayer) then
                        TriggerClientEvent("tratamento",nplayer)
                        TriggerClientEvent("Notify",source,"sucesso","Tratamento no paciente iniciado com sucesso.",10000)
...
--         embeds = {
--             {
--                 description = '**SERVIDOR ONLINE:**\n\nAperte F8 e cole: connect deltacity.servegame.com',
