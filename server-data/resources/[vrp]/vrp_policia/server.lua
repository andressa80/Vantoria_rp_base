...
"https://discord.com/api/webhooks/838788240767975444/EALG_Wnt-8Ol-v6jxRdFSCaIW9joKwJliYFStv7Ddm31s-DSgtAe1f8Mb1egTdp28oKo"
local webhookprender =
"https://canary.discord.com/api/webhooks/878623283631771728/oLarHPjUiqzSWEpjV4ZssCAIW4qTPr4U76WlAK7X4XgVcOtsC8WwtwvzNkp--ltVINWU"
local webhookmultas =
"https://canary.discord.com/api/webhooks/878623574863282186/UomP3KL3jsbC0_9ry2piC7bLd3vgTjX8OynRh_eYA46fSBnlmik_ell3lRHnWevisB26"
local webhookocorrencias =
"https://canary.discord.com/api/webhooks/878623731176575016/uarvcep5j53-Dhs1My_fZcsSO5yHZEOoGiqJsywzC0gHnFmKqc2HHNlD6BSdge-9IKIt"
local webhookdetido =
"https://canary.discord.com/api/webhooks/878624978994610237/gDo8JnBJZ9r4CqSY-NwEuXQ-yEOJ7Sd7wM6T6f-1Tkg1V966_zgyfpyuKCU9lS-wSgr0"
local webhookponto =
"https://discord.com/api/webhooks/852276964604772452/PHScB7f1dDNwFp0XQCsLAi7iM_3AYjfQ0_6-mSQTzbpRh8Tx8wd9YBU8cNRQsqP3c0Gr"
...
"https://canary.discord.com/api/webhooks/878623880275722280/z5EXVRF5vxgLf_5DkmBNoV7VMTJQ-DoqQ6fhQMSw4uaXquk5eGGuk72tQ52rMTzhWEqI"
		...
		if vRP.terPemissao(user_id, "policia.permissao") then
			...
			for l, w in pairs(soldado) do
				...
				if player and player ~= uplayer then
					async(function()
						local id = idgens:gen()
						policia[id] = vRPclient.addBlip(player, x, y, z, 153, 84,
							"Localização de " .. identity.name .. " " .. identity.firstname, 0.5, false)
						TriggerClientEvent("NotifyPush", player,
							{ code = 31, title = "Localização recebida de <b>" ..
							identity.name .. " " .. identity.firstname .. "</b>.", x = x, y = y, z = z, badge =
							"Localização." })
...
RegisterCommand('toogle', function(source, args, rawCommand)
	...
	if vRP.terPemissao(user_id, "recruta.servico") then
		...
		SendWebhookMessage(webhookponto,
			"```prolog\n[RECRUTA]: » " ..
			user_id ..
			" " ..
			identity.name ..
...
RegisterCommand('multar', function(source, args, rawCommand)
	...
	if vRP.terPemissao(user_id, "policia.permissao") then
		...
		TriggerClientEvent("Notify", nplayer, "importante",
			"Você foi multado em <b>$" .. vRP.format(parseInt(valor)) .. " dólares</b>.<br><b>Motivo:</b> " ..
			motivo .. ".")
			...
			if vehicleInfo and #vehicleInfo > 0 then
				if (vehicleInfo[1].detido and vehicleInfo[1].detido == 0) or (vehicleInfo[1].arrest and vehicleInfo[1].arrest == 0) then
					...
					TriggerClientEvent("Notify", source, "importante",
						"Você recebeu <b>$" .. vRP.format(parseInt(randmoney)) .. " dólares</b> de bonificação.")
					TriggerClientEvent("Notify", nplayer, "importante",
						"Seu Veículo foi <b>Detido</b>.<br><b>Motivo:</b> " .. motivo .. ".")
					vRPclient.playSound(source, "Hack_Success", "DLC_HEIST_BIOLAB_PREP_HACKING_SOUNDS")
				end
			end
		end
	...
	if vRP.terPemissao(user_id, "admin.permissao") or vRP.terPemissao(user_id, "policia.permissao") then
		...
		if player then
			...
			TriggerClientEvent("Notify", source, "importante",
				"Você recebeu <b>$" .. vRP.format(parseInt(randmoney)) .. " dólares</b> de bonificação.")
			TriggerClientEvent("Notify", nplayer, "importante",
				"Você foi preso por <b>" .. vRP.format(parseInt(args[2])) ..
				" meses</b>.<br><b>Motivo:</b> " .. crimes .. ".")
			vRPclient.playSound(source, "Hack_Success", "DLC_HEIST_BIOLAB_PREP_HACKING_SOUNDS")
		end
	end
end)
		...
		if args[1] then
			...
			if nplayer == nil then
				TriggerClientEvent("Notify", source, "aviso",
					"Passaporte <b>" .. vRP.format(args[1]) .. "</b> indisponível no momento.")
				return
			end
			...
			if nuser_id then
				...
				local banco = vRP.getBankMoney(nuser_id)
				TriggerClientEvent("Notify", nplayer, "importante",
					"Seu documento está sendo verificado por <b>" .. identityv.name .. " " ..
					identityv.firstname .. "</b>.")
				vRPclient.setDiv(source, "completerg",
					".div_completerg { background-color: rgba(0,0,0,0.60); font-size: 13px; font-family: arial; color: #fff; width: 420px; padding: 20px 20px 5px; bottom: 8%; right: 2.5%; position: absolute; border: 1px solid rgba(255,255,255,0.2); letter-spacing: 0.5px; } .local { width: 220px; padding-bottom: 15px; float: left; } .local2 { width: 200px; padding-bottom: 15px; float: left; } .local b, .local2 b { color: #d1257d; }",
					"<div class=\"local\"><b>Nome:</b> " ..
					identity.name ..
						...
						if amount > 0 then
							...
							if item_name then
								if vRP.tryGetInventoryItem(nuser_id, idname, amount, true) then
									...
									table.insert(itens_apreendidos,
										"[ITEM]: " .. vRP.itemNameList(idname) .. " [QUANTIDADE]: " .. amount)
								end
							end
						end
					end
...
RegisterCommand('anuncio', function(source, args, rawCommand)
	...
	if vRP.terPemissao(user_id, "admin.permissao") or vRP.terPemissao(user_id, "policia.permissao") or vRP.terPemissao(user_id, "conce.permissao") or vRP.terPemissao(user_id, "news.permissao") then
		...
		vRPclient.setDiv(-1, "anuncio",
			...
			"<bold>" .. mensagem .. "</bold><br><br>Mensagem enviada por: " .. identity.name .. " " .. identity
			.firstname)
		SetTimeout(30000, function()
