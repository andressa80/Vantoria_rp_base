...
AddEventHandler('deletarveiculo', function(vehicle)
    if IsEntityAVehicle(vehicle) then
        TriggerServerEvent("vrp_garages_id", VehToNet(vehicle), GetVehicleEngineHealth(vehicle),
            GetVehicleBodyHealth(vehicle), GetVehicleFuelLevel(vehicle))
        ...
        if canPedBeUsed(ped) and distance < 30.0 and (distanceFrom == nil or distance < distanceFrom) then
            ...
            if IsEntityTouchingEntity(GetPlayerPed(-1), ped) then
                ...
                DrawText3Ds(pos["x"], pos["y"], pos["z"],
                    "Ped: " ..
                    ped ..
                    " Model: " .. GetEntityModel(ped) .. " Relationship HASH: " .. GetPedRelationshipGroupHash(ped))
            end
...
Citizen.CreateThread(function()
    while true do
        ...
        if dickheaddebug then
            ...
            drawTxtS(0.8, 0.54, 0.4, 0.4, 0.30, "Attached Ent: " .. GetEntityAttachedTo(GetPlayerPed(-1)), 55, 155, 55,
                255)
            drawTxtS(0.8, 0.56, 0.4, 0.4, 0.30, "Health: " .. GetEntityHealth(GetPlayerPed(-1)), 55, 155, 55, 255)
            drawTxtS(0.8, 0.58, 0.4, 0.4, 0.30, "H a G: " .. GetEntityHeightAboveGround(GetPlayerPed(-1)), 55, 155, 55,
                255)
