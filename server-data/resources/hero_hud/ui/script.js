...
window.addEventListener('message', ({data: c}) => {
    if (c.action == 'toggleHUD') {
        const hudElements = document.querySelectorAll('.info, .velocimeter, .player, .notifys, .notifysAdm, .progressbar, .cupom, .chat, hotkey');
...
// Tecla U para esconder/mostrar HUD
...
function toggleHUD() {
    ...
    hudElements.forEach(element => {
        if (element.classList.contains('hud-visible') || !element.classList.contains('hud-hidden')) {
            element.classList.remove('hud-visible');
            element.classList.add('hud-hidden');
        } else {
            element.classList.remove('hud-hidden');
